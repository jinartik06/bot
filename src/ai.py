...
class IdeaAI:
    ...
    async def check_t_one_connection(self) -> tuple[bool, str]:
        if not self.can_transcribe():
            return False, "T-one transcription is not configured"
        timeout = aiohttp.ClientTimeout(
            total=min(15, self.config.t_one_connect_timeout_seconds + self.config.t_one_receive_timeout_seconds),
            connect=min(10, self.config.t_one_connect_timeout_seconds),
            sock_connect=min(10, self.config.t_one_connect_timeout_seconds),
        )
        try:
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.ws_connect(self.config.t_one_ws_url, heartbeat=5) as ws:
                    message = await ws.receive(timeout=10)
                    if message.type != aiohttp.WSMsgType.TEXT:
                        return False, f"unexpected websocket message type: {message.type.name}"
                    try:
                        data = json.loads(message.data)
                    except json.JSONDecodeError:
                        return False, "T-one returned non-JSON websocket message"
                    event = data.get("event")
                    if event != "ready":
                        return False, f"unexpected T-one event: {event or 'empty'}"
