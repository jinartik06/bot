...
class IdeaAI:
    ...
    def _t_one_chunks(self, pcm: bytes) -> list[bytes]:
        chunks: list[bytes] = []
        for offset in range(0, len(pcm), self.T_ONE_CHUNK_BYTES):
            chunk = pcm[offset : offset + self.T_ONE_CHUNK_BYTES]
            if len(chunk) < self.T_ONE_CHUNK_BYTES:
                chunk += b"\x00" * (self.T_ONE_CHUNK_BYTES - len(chunk))
            chunks.append(chunk)
        return chunks
