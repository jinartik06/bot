...
async def main() -> None:
    ...
    ai = IdeaAI(config)
    logger.info(
        "Voice config: transcriber=%s t_one_ws_url=%s send_chunk_seconds=%s timeouts=%s/%s/%s voice_timeout=%s ffmpeg=%s",
        config.voice_transcriber,
        config.t_one_ws_url,
        config.t_one_send_chunk_seconds,
        config.t_one_connect_timeout_seconds,
        config.t_one_receive_timeout_seconds,
        config.t_one_total_timeout_seconds,
        config.voice_processing_timeout_seconds,
        config.ffmpeg_binary,
    )
    if ai.uses_t_one_transcriber():
        t_one_ok, t_one_detail = await ai.check_t_one_connection()
        if t_one_ok:
            logger.info("T-one websocket check succeeded: %s", t_one_detail)
        else:
            logger.warning("T-one websocket check failed: %s", t_one_detail)
