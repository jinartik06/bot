...
async def transcribe_with_voice_runtime_lock(
    ...
    while True:
        ...
        if not notified:
            ...
            await message.answer("Распознавание голоса сейчас занято. Поставил это сообщение в очередь.")
...
async def capture_idea(message: Message, state: FSMContext, bot: Bot, db: Database, ai: IdeaAI, config: Config) -> None:
    ...
    if message.voice or message.audio:
        ...
        if ai.voice_is_busy():
            await message.answer("Распознавание голоса ещё занято предыдущим сообщением. Поставил это сообщение в очередь.")
