...
class Database:
    ...
    async def try_acquire_runtime_lock(self, name: str, owner: str, ttl_seconds: float) -> bool:
        now = datetime.now(timezone.utc)
        now_ts = now.timestamp()
        expires_at = now_ts + ttl_seconds
        cur = await self.db.execute(
            """
            INSERT INTO runtime_locks (name, owner, expires_at, updated_at)
            VALUES (?, ?, ?, ?)
            ...
            """,
            (name, owner, expires_at, now.isoformat(), now_ts),
        )
        await self.db.commit()
        return cur.rowcount > 0

    async def release_runtime_lock(self, name: str, owner: str) -> bool:
        cur = await self.db.execute(
